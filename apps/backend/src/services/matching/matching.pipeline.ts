...
export async function runForUser(userId: string): Promise<void> {
  ...
  const rpcArgs: { query_embedding: string; match_count: number; location_filter?: string; salary_min?: number } = {
    ...
    query_embedding: profile.embedding as unknown as string,
    match_count: 30,
  };
  if (locationFilter) rpcArgs.location_filter = locationFilter;
  if (salaryMin !== undefined) rpcArgs.salary_min = salaryMin;

  const { data: jobs, error: rpcError } = await supabase.rpc('match_jobs_for_user', rpcArgs);
  ...
  // Build one batch prompt using structured_summary (not full description)
  ...
  const prompt = `Candidate: ${profile.raw_profile_text}
